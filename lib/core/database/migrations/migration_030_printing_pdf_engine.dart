...
/// R3.16.1 Printing/PDF runtime.
///
/// Printing remains application-neutral. System DocTypes are the public
/// configuration surface while renderers/adapters are native runtime units.
class Migration030PrintingPdfEngine extends SqlDatabaseMigration {
  const Migration030PrintingPdfEngine();

  @override
  int get version => 30;

  @override
  String get name => 'printing_pdf_engine';

  @override
  void apply(CommonDatabase database) {
    // SqlDatabaseMigration intentionally splits ordinary migration SQL on
    // statement semicolons. SQLite trigger bodies contain their own semicolons,
    // so execute those definitions atomically after the regular v30 SQL.
    super.apply(database);
    for (final triggerSql in _validationTriggers) {
      database.execute(triggerSql);
    }
  }

  static const List<String> _validationTriggers = [
    r'''CREATE TRIGGER IF NOT EXISTS trg_print_formats_validate_insert
...
BEGIN
  ...
  END;
END;''',
    r'''CREATE TRIGGER IF NOT EXISTS trg_print_formats_validate_update
